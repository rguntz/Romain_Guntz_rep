...
- My current project focuses on dexterous manipulation for assembly tasks. The approach builds on ideas introduced in the paper “Imitation Bootstrapped Reinforcement Learning” by Hengyuan Hu et al. It starts by learning a policy from expert demonstrations, which is then reused as a guiding prior to improve exploration efficiency and learning stability during reinforcement learning, in parallel with directly incorporating demonstrations into the RL pipeline. We then extend this framework by training a diffusion-based policy, which is subsequently fine-tuned using imitation-bootstrapped reinforcement learning. For this final stage, either a residual policy or a steering diffusion policy approach will be adopted. Stay tuned!
