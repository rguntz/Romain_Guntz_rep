...
class Segment(ABC):
    ...
    def __init__(
        ...
        self.type = segment_type  ## Straight line or right of left
...
class Line(Segment):
    ...
    def __init__(self, start_config: SE2Transform, end_config: SE2Transform, gear: Gear = Gear.FORWARD):
        ...
        if np.abs(self.length) >= 1e-8:
            self.direction = (
                end_config.p - start_config.p
            ) / self.length  ## Computed as a unit vector pointing from start to end. If the length is effectively zero, the direction defaults to an identity transformation.
        else:
