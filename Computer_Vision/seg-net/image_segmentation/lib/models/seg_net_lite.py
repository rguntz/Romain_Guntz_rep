...
class SegNetLite(nn.Module):
    ...
    def __init__(self, kernel_sizes=[3, 3, 3, 3], down_filter_sizes=[32, 64, 128, 256],
        ...
        for i in range(4):
            ...
            layers_conv_up.append(nn.Conv2d(
                in_channels=(256 if i == 0 else up_filter_sizes[i - 1]),
    ...
    def forward(self, x):
        ...
        for i in range(self.num_down_layers):
            ...
            x = self.relu(x)
            x, indices = self.layers_pooling[i](x)  # Get both pooled values and indices
            pooling_indices.append(indices)        # Store indices for unpooling

        # Upsampling
        for i in range(self.num_up_layers):
            indices = pooling_indices[self.num_up_layers - 1 - i]  # Reverse order for unpooling
            x = self.layers_unpooling[i](x, indices)              # Pass indices to unpooling
