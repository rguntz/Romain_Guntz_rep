...
def least_square(x,y):
	...
	# numpy.linalg.lstsq is used to compute the least-squares solution
	solution, _, _, _ = np.linalg.lstsq(A, y, rcond=None)

	k, b = solution  # We select the slope and the intercept of the line
...
def num_inlier(x,y,k,b,n_samples,thres_dist):
	...
	# aAbsolute distance of each point from the line
	distances = np.abs(y - y_pred)

	# Select where the distance is inferior to the threshold 
	mask = distances < thres_dist
	num = np.sum(mask)  # Count the number of points that have a distance inferor to the threshold (number of inliers)
...
def ransac(x,y,iter,n_samples,thres_dist,num_subset):
	...
	# Loop of the size of the parameter "iter"
	for _ in range(iter):
		indices = random.sample(range(n_samples), num_subset)
		x_subset = x[indices]
		y_subset = y[indices]
		
		# Do the least-squares solution for this sample
		k, b = least_square(x_subset, y_subset)
		
		# Do the number of inliers and the mask
