...
def EstimateEssentialMatrix(K, im1, im2, matches):
  ...
  E_hat = vectorized_E_hat.reshape(3, 3)  ## It must be a 3 by 3 
  ...
  S = np.array([1, 1, 0]) ## We choose two singular values equal to 1 (as it is superior to 0), last one is equal to 0 as mentionned in the instructions (third singular value is 0)
  E = U @ np.diag(S) @ Vt
...
def TriangulatePoints(K, im1, im2, matches):
  ...
  # Second camera
  camera_coords2 = (R2 @ points3D.T + t2.reshape(3, 1)).T
  valid_indices2 = camera_coords2[:, 2] > 0
  im1_corrs = im1_corrs[valid_indices2]
  im2_corrs = im2_corrs[valid_indices2]
  points3D = points3D[valid_indices2]
...
def EstimateImagePose(points2D, points3D, K):  
  ...
  normalized_points2D = np.linalg.inv(K) @ np.hstack((points2D, np.ones((points2D.shape[0], 1)))).T
  normalized_points2D = normalized_points2D[:2, :].T  # Converting to 2D 
...
def TriangulateImage(K, image_name, images, registered_images, matches):
  ...
  for reg_image_name in registered_images:
      ...
      kps1 = image.kps[match[:, 0]]  
      kps2 = images[reg_image_name].kps[match[:, 1]]  

      # camera poses and translation t 
      R1, t1 = image.GetPose()
      ...
      # Triangulation : 
      triangulated_points = TriangulatePoints(kps1, kps2, P1, P2)

      # Add valid points 
      valid_points = triangulated_points[:, 3] > 1e-6 
      triangulated_points = triangulated_points[valid_points]
      points3D = np.vstack((points3D, triangulated_points[:, :3] / triangulated_points[:, 3][:, None]))  

      # Track correspondences for the two images
      local_corrs = np.where(valid_points)[0]
      corrs[reg_image_name] = match[local_corrs, 1]  
      corrs[image_name] = match[local_corrs, 0] 
