...
def filter_keypoints(img, keypoints, patch_size = 9):
    ...
    half_patch = patch_size // 2  # Calculate half patch size

    # Filter out keypoints that are too close to the edges
    filtered_keypoints = []
    for kp in keypoints:
        x, y = kp  # Get the x, y coordinates of the keypoint
        # Check if the keypoint is far enough from the borders
        if (x >= half_patch and x < width - half_patch and
            y >= half_patch and y < height - half_patch):
